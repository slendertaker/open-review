 ...
 *
 * webhookSecret is generated by the caller and included in hook_attributes so
 * GitHub signs deliveries with it from the moment the App is created -- no
 * manual webhook secret entry is ever needed (T-05-09 successor).
 */
function buildManifest(baseUrl: string, appName: string, webhookSecret: string): string {
  const manifest = {
    name: appName,
    url: baseUrl,
    hook_attributes: {
...
export async function registerGithubRoutes(
  ...
  fastify.get(
    ...
    async (req: Req, reply: Rep) => {
      ...
      // Generate and persist the webhook secret now, before the operator ever
      // leaves for GitHub. This guarantees it is set the moment the App can
      // receive deliveries -- no manual webhook secret entry, and no reliance
      // on GitHub echoing one back in the callback response.
      const webhookSecret = randomBytes(32).toString('hex');
      setSetting('webhook_secret', webhookSecret);

      const manifestJson = buildManifest(baseUrl, appName, webhookSecret);
