 ...
 *         plain setting:     webhook_secret returned by GitHub (guarded for null -- Pitfall 1)
 ...
 */
export async function buildInstallGroups(store: ConfigStore, machineKey: Buffer): Promise<InstallGroup[]> {
...
export async function registerGithubRoutes(
    ...
    async (req: Req, reply: Rep) => {
      ...
      if (isHttp) {
        ...
        const flash = renderFlash(
          'error',
          'Set a public HTTPS domain in Access before connecting GitHub. GitHub cannot create an App with a localhost or HTTP webhook URL.',
        );
  ...
  fastify.get(
    ...
    async (req: Req, reply: Rep) => {
      ...
      try {
        ...
        // Redirect to the install step. The operator visits GitHub's account/org
        // picker, then configures repository access there (D5-04).
  ...
  // -------------------------------------------------------------------------
  // GET /dashboard/github/install -- redirect to the GitHub App target picker
  // -------------------------------------------------------------------------
  fastify.get(
    ...
    async (req: Req, reply: Rep) => {
      ...
      // D5-04: /select_target lets the operator choose a personal account or
      // organization. /installations/new can jump straight to an existing
      // installation settings page, which hides the account/org chooser.
      return reply.redirect(`https://github.com/apps/${slug}/installations/select_target`);
    },
  );
  ...
  fastify.get(
    ...
    async (req: Req, reply: Rep) => {
      ...
      if (!connected) {
        ...
        const appName = getSetting('github_app_name') ?? '';
        const htmlUrl = getSetting('github_app_html_url') ?? '';
        // Repo listing/toggling now lives on the Repositories page
        // (routes-repo-settings.ts) -- no GitHub API call needed here.
        const onboardingStep = store.repos.length > 0 ? 3 : 2;
        githubData = { csrfToken, connected: true, onboardingStep, slug, appName, htmlUrl, flash: '' };
      }
